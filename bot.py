...
# Главное меню – добавлена кнопка КБЖУ
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📊 КБЖУ", callback_data="kbzhu")]
    ])

# Возвращает текст кнопки отчёта (без изменений)
...
async def start_free_course(msg, ctx: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    # Для АНАСТАСИИ бесплатный курс доступен только при выборе женского дома (условие по ТЗ)
    if instructor == "anastasiya":
    ...
    free_day_key = f"free_day_{instructor}"
    day = ctx.user_data[user_id].get(free_day_key, 1)
    if day > 5:
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(get_report_button_text(ctx, user_id), callback_data=f"send_report_{instructor}_day_{day}")]
    ])
    try:
...
async def handle_send_report(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    parts = query.data.split("_")  # формат: send_report_<instructor>_day_<day>
    if len(parts) < 5:
        return await query.message.reply_text("Ошибка данных отчета.")
    instructor = parts[2]
    try:
        day = int(parts[4])
    except:
        return await query.message.reply_text("Ошибка дня отчета.")
    reports_key = f"free_reports_{instructor}"
    if ctx.user_data[user_id].get(reports_key, {}).get(day):
        return await query.message.reply_text(f"Вы уже отправили отчет за день {day}.")
    waiting_key = f"waiting_video_{instructor}"
    ctx.user_data[user_id][waiting_key] = day
...
async def handle_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    waiting_key = f"waiting_video_{instructor}"
    if waiting_key in ctx.user_data[user_id]:
        data = ctx.user_data[user_id][waiting_key]
        # Если это видео по платному курсу (объект tuple: ("paid", day))
        if isinstance(data, tuple) and data[0] == "paid":
            paid_day = data[1]
            await ctx.bot.send_message(chat_id=GROUP_ID, text=f"Платный видео-отчет от {update.message.from_user.first_name} (ID: {user_id}) за день {paid_day}.")
            await ctx.bot.send_video(chat_id=GROUP_ID, video=update.message.video.file_id)
            score_key = f"score_{instructor}"
            ctx.user_data[user_id][score_key] += 30
            del ctx.user_data[user_id][waiting_key]
            if paid_day < 5:
                next_day = paid_day + 1
                await update.message.reply_text(
                    f"Отчет за платный день {paid_day} принят! 🎉\nВаши баллы: {ctx.user_data[user_id][score_key]}.\nГотовы к следующему дню ({next_day})? ➡️",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"➡️ День {next_day}", callback_data=f"paid_next_{instructor}")]])
                )
            else:
                await update.message.reply_text(f"Поздравляем! Вы завершили платный курс! 🎉\nВаши баллы: {ctx.user_data[user_id][score_key]}.", reply_markup=main_menu())
                ctx.user_data[user_id].pop(f"paid_day_{instructor}", None)
        # Обработка видео для бесплатного курса
        elif isinstance(data, int):
            day = data
            await ctx.bot.send_message(chat_id=GROUP_ID, text=f"Видео-отчет от {update.message.from_user.first_name} (ID: {user_id}) за день {day}.")
            await ctx.bot.send_video(chat_id=GROUP_ID, video=update.message.video.file_id)
            reports_key = f"free_reports_{instructor}"
            if reports_key not in ctx.user_data[user_id]:
                ctx.user_data[user_id][reports_key] = {}
            ctx.user_data[user_id][reports_key][day] = True
            score_key = f"score_{instructor}"
            ctx.user_data[user_id][score_key] += 60
            del ctx.user_data[user_id][waiting_key]
            free_day_key = f"free_day_{instructor}"
            if day < 5:
                new_day = day + 1
                ctx.user_data[user_id][free_day_key] = new_day
                ctx.user_data[user_id][waiting_key] = new_day
                await update.message.reply_text(
                    f"Отчет за день {day} принят! 🎉\nВаши баллы: {ctx.user_data[user_id][score_key]}.\nГотовы к следующему дню ({new_day})? ➡️",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"➡️ День {new_day}", callback_data=f"next_free_{instructor}")]])
                )
            else:
                status_key = f"status_{instructor}"
                ctx.user_data[user_id][status_key] = statuses[1]
...
async def handle_free_course_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    # Если нажата кнопка для запуска бесплатного курса или перехода на следующий день
    if query.data.startswith("free_course") or query.data.startswith(f"next_free_{instructor}"):
        if instructor == "anastasiya" and ("gender" not in ctx.user_data[user_id] or "program" not in ctx.user_data[user_id]):
...
# Логика выбора пола и программы (для бесплатного курса, в частности для АНАСТАСИИ)
...
async def handle_program(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # После выбора программы для АНАСТАСИИ сразу запускаем бесплатный курс
    await start_free_course(query.message, ctx, user_id)
    ...
    try:
        ...
        if ctx.args:
            try:
                ref = int(ctx.args[0])
                if ref != user_id:
                    # Здесь можно начислить реферальный бонус (если требуется)
                    try:
        ...
        # При старте ещё не выбран инструктор – предлагаем выбор
        kb = InlineKeyboardMarkup([
...
async def handle_instructor_selection(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "instructor_1":
        ...
        # Инициализируем индивидуальные данные для Евгения
        for key, default in [("score", 0), ("free_day", 1), ("paid_day", 1)]:
            if f"{key}_evgeniy" not in ctx.user_data[user_id]:
                ctx.user_data[user_id][f"{key}_evgeniy"] = default
        if "free_reports_evgeniy" not in ctx.user_data[user_id]:
            ctx.user_data[user_id]["free_reports_evgeniy"] = {}
        if "status_evgeniy" not in ctx.user_data[user_id]:
            ctx.user_data[user_id]["status_evgeniy"] = statuses[0]
        if "challenge_evgeniy" not in ctx.user_data[user_id]:
            ctx.user_data[user_id]["challenge_evgeniy"] = {"current_day": 1}
        ...
        # Инициализируем индивидуальные данные для АНАСТАСИИ
        for key, default in [("score", 0), ("free_day", 1), ("paid_day", 1)]:
            if f"{key}_anastasiya" not in ctx.user_data[user_id]:
                ctx.user_data[user_id][f"{key}_anastasiya"] = default
        if "free_reports_anastasiya" not in ctx.user_data[user_id]:
            ctx.user_data[user_id]["free_reports_anastasiya"] = {}
        if "status_anastasiya" not in ctx.user_data[user_id]:
            ctx.user_data[user_id]["status_anastasiya"] = statuses[0]
        if "challenge_anastasiya" not in ctx.user_data[user_id]:
            ctx.user_data[user_id]["challenge_anastasiya"] = {"current_day": 1}
...
# --------------------- Питание, реферал, челленджи и т.п. ---------------------
...
async def handle_buy_nutrition_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    # Здесь можно использовать общий счёт (если не зависит от тренера)
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    score_key = f"score_{instructor}"
    if ctx.user_data[user_id].get(score_key, 0) >= 300:
        ctx.user_data[user_id][score_key] -= 300
...
async def handle_challenges(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    challenge_key = f"challenge_{instructor}"
    score_key = f"score_{instructor}"
    if ctx.user_data[user_id].get(challenge_key):
        await send_challenge_task(query.message, user_id, instructor)
    elif ctx.user_data[user_id].get(score_key, 0) >= 300:
        kb = InlineKeyboardMarkup([
...
async def buy_challenge(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data[user_id].get(score_key, 0) >= 300:
        ctx.user_data[user_id][score_key] -= 300
        challenge_key = f"challenge_{instructor}"
        ctx.user_data[user_id][challenge_key] = {"current_day": 1}
...
async def send_challenge_task(message, user_id: int, instructor: str):
    challenge_key = f"challenge_{instructor}"
    day = ctx.user_data[user_id][challenge_key]["current_day"]
    exercises = {
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Следующий день", callback_data=f"challenge_next_{instructor}")]]
...
async def handle_challenge_next_day(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    challenge_key = f"challenge_{instructor}"
    if not ctx.user_data[user_id].get(challenge_key):
        return await query.answer("Сначала купите челлендж! 🚧")
    day = ctx.user_data[user_id][challenge_key]["current_day"]
    if day < 5:
        ctx.user_data[user_id][challenge_key]["current_day"] = day + 1
        await send_challenge_task(query.message, user_id, instructor)
    else:
        await query.message.reply_text("Поздравляем, вы завершили челлендж! 🎉", reply_markup=main_menu())
        ctx.user_data[user_id].pop(challenge_key, None)
...
async def handle_paid_course(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    score_key = f"score_{instructor}"
    discount = min(ctx.user_data[user_id][score_key] * 2, 600)
    price = 2000 - discount
    ...
    waiting_receipt_key = f"waiting_receipt_{instructor}"
    ctx.user_data[user_id][waiting_receipt_key] = True
...
async def handle_send_receipt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 2:
        return await query.message.reply_text("Ошибка данных.")
    instructor = parts[1]
    waiting_receipt_key = f"waiting_receipt_{instructor}"
    ctx.user_data[user_id][waiting_receipt_key] = True
...
async def handle_receipt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    waiting_receipt_key = f"waiting_receipt_{instructor}"
    if not ctx.user_data[user_id].get(waiting_receipt_key):
    ...
    await ctx.bot.send_message(chat_id=GROUP_ID, text=f"🧾 Чек от {update.message.from_user.first_name} (ID: {user_id}). Подтвердите оплату.")
...
async def confirm_payment(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    parts = query.data.split("_")
    if len(parts) < 3:
        return await query.message.reply_text("Ошибка данных подтверждения.")
    user_id = int(parts[2])
    instructor = parts[3]
    status_key = f"status_{instructor}"
    ctx.user_data[user_id][status_key] = statuses[2]
    waiting_receipt_key = f"waiting_receipt_{instructor}"
    if waiting_receipt_key in ctx.user_data[user_id]:
        del ctx.user_data[user_id][waiting_receipt_key]
    ...
    if instructor == "evgeniy":
        ...
        paid_day_key = f"paid_day_{instructor}"
        ctx.user_data[user_id][paid_day_key] = 1
        day1_ex = [
        ...
        kb_day1 = InlineKeyboardMarkup([[InlineKeyboardButton("📹 Отправить отчет", callback_data=f"paid_video_{instructor}_day_1")]])
...
async def handle_paid_next_day(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    instructor = ctx.user_data[user_id].get("instructor", "evgeniy")
    paid_day_key = f"paid_day_{instructor}"
    paid_day = ctx.user_data[user_id].get(paid_day_key, 1)
    if paid_day < 5:
        next_day = paid_day + 1
        ctx.user_data[user_id][paid_day_key] = next_day
        paid_program = {
            1: [
                "Махи назад с утяжелителями 3х25+5 https://t.me/c/2241417709/337/338",
                "Выпады 3х30 шагов х 2кг https://t.me/c/2241417709/157/158",
        ...
        ex = paid_program.get(next_day, [])
        text = f"📚 **Платный курс: День {next_day}** 📚\n\n" + "\n".join(ex) + "\n\nОтправьте видео-отчет за день! 🎥"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("📹 Отправить отчет", callback_data=f"paid_video_{instructor}_day_{next_day}")]])
...
async def handle_send_paid_report(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    parts = query.data.split("_")  # формат: paid_video_<instructor>_day_<day>
    if len(parts) < 4:
        return await query.message.reply_text("Ошибка данных.")
    instructor = parts[2]
    try:
        ...
        return await query.message.reply_text("Ошибка дня отчета.")
    waiting_key = f"waiting_video_{instructor}"
    ctx.user_data[user_id][waiting_key] = ("paid", day)
    await query.message.reply_text(f"Пожалуйста, отправьте видео-отчет за платный день {day} 🎥")
...
async def handle_my_cabinet(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    score_key = f"score_{instructor}"
    status_key = f"status_{instructor}"
    score = ctx.user_data[user_id].get(score_key, 0)
    status = ctx.user_data[user_id].get(status_key, statuses[0])
...
async def handle_kbzhu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    # Инициализируем данные для диалога расчета КБЖУ
    ctx.user_data[user_id]["kbzhu"] = {}
    ctx.user_data[user_id]["kbzhu_step"] = 1
    await query.message.reply_text("Расчет КБЖУ.\nВведите ваш пол (male/female):")

async def handle_kbzhu_response(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if "kbzhu_step" not in ctx.user_data[user_id]:
        return  # не ведется диалог КБЖУ
    step = ctx.user_data[user_id]["kbzhu_step"]
    kbzhu_data = ctx.user_data[user_id].setdefault("kbzhu", {})
    text = update.message.text.strip()
    if step == 1:
        kbzhu_data["gender"] = text.lower()
        ctx.user_data[user_id]["kbzhu_step"] = 2
        await update.message.reply_text("Введите ваш вес (кг):")
    elif step == 2:
        try:
            kbzhu_data["weight"] = float(text)
        except:
            return await update.message.reply_text("Пожалуйста, введите корректное число для веса.")
        ctx.user_data[user_id]["kbzhu_step"] = 3
        await update.message.reply_text("Введите ваш рост (см):")
    elif step == 3:
        try:
            kbzhu_data["height"] = float(text)
        except:
            return await update.message.reply_text("Пожалуйста, введите корректное число для роста.")
        ctx.user_data[user_id]["kbzhu_step"] = 4
        await update.message.reply_text("Введите ваш возраст:")
    elif step == 4:
        try:
            kbzhu_data["age"] = int(text)
        except:
            return await update.message.reply_text("Пожалуйста, введите корректный возраст.")
        ctx.user_data[user_id]["kbzhu_step"] = 5
        await update.message.reply_text("Введите уровень активности (низкий, средний, высокий):")
    elif step == 5:
        kbzhu_data["activity"] = text.lower()
        ctx.user_data[user_id]["kbzhu_step"] = 6
        await update.message.reply_text("Введите вашу цель (похудеть, набрать, поддержание):")
    elif step == 6:
        kbzhu_data["goal"] = text.lower()
        # Расчет по формуле (Mifflin-St Jeor + коррекция по активности и цели)
        weight = kbzhu_data["weight"]
        height = kbzhu_data["height"]
        age = kbzhu_data["age"]
        gender = kbzhu_data["gender"]
        activity = kbzhu_data["activity"]
        goal = kbzhu_data["goal"]
        if gender == "male":
        ...
        if activity == "низкий":
            ...
            af = 1.55
        elif activity == "высокий":
            af = 1.725
        else:
            af = 1.2
        tdee = bmr * af
        if goal == "похудеть":
            tdee_adjusted = tdee - 500
        elif goal == "набрать":
            tdee_adjusted = tdee + 500
        else:
            tdee_adjusted = tdee
        proteins = (tdee_adjusted * 0.3) / 4
        fats = (tdee_adjusted * 0.25) / 9
        carbs = (tdee_adjusted * 0.45) / 4
        result = (f"Ваш расчет КБЖУ:\n"
                  f"Калории: {tdee_adjusted:.0f} ккал\n"
                  f"Белки: {proteins:.1f} г\n"
                  f"Жиры: {fats:.1f} г\n"
                  f"Углеводы: {carbs:.1f} г")
        await update.message.reply_text(result)
        # Очищаем данные диалога
        del ctx.user_data[user_id]["kbzhu"]
        del ctx.user_data[user_id]["kbzhu_step"]
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_free_course_callback, pattern="^(free_course|next_free_.*)$"))
    ...
    app.add_handler(CallbackQueryHandler(handle_send_report, pattern=r"^send_report_\w+_day_\d+$"))
    app.add_handler(CallbackQueryHandler(handle_challenges, pattern="^challenge_menu$"))
    app.add_handler(CallbackQueryHandler(buy_challenge, pattern=r"^buy_challenge_.*$"))
    app.add_handler(CallbackQueryHandler(handle_paid_course, pattern="^paid_course$"))
    app.add_handler(CallbackQueryHandler(handle_send_receipt, pattern=r"^send_receipt_.*$"))
    app.add_handler(CallbackQueryHandler(confirm_payment, pattern=r"^confirm_payment_\d+_\w+$"))
    app.add_handler(CallbackQueryHandler(handle_send_paid_report, pattern=r"^paid_video_\w+_day_\d+$"))
    app.add_handler(CallbackQueryHandler(handle_paid_next_day, pattern=r"^paid_next_\w+$"))
    ...
    app.add_handler(CallbackQueryHandler(handle_challenge_next_day, pattern=r"^challenge_next_\w+$"))
    app.add_handler(CallbackQueryHandler(handle_back, pattern="^back$"))
    app.add_handler(CallbackQueryHandler(handle_kbzhu, pattern="^kbzhu$"))
    app.add_handler(MessageHandler(filters.VIDEO, handle_video))
    app.add_handler(MessageHandler(filters.PHOTO, handle_receipt))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_kbzhu_response))
