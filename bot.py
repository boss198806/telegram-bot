...
user_reports_sent = {}  # Отслеживание отправленных видео-отчетов (ключ – день)
user_waiting_for_video = {}  # Ожидание видео-отчёта (для бесплатного и платного курсов)
...
async def start_free_course(msg, ctx: ContextTypes.DEFAULT_TYPE, user_id: int):
    # Для тренеров "evgeniy" и "anastasiya" ведём отдельный прогресс
...
# --------------------- Логика для бесплатного курса (пол/программа) ---------------------
...
# Покупка КБЖУ (исправлено: баланс берется в зависимости от выбранного тренера)
