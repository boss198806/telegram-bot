...
# Обработчики пола/программы для бесплатного курса
...
async def handle_instructor_selection(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    user_id = query.from_user.id
    logger.info("handle_instructor_selection: data=%s, user_id=%s", data, user_id)
    await query.answer()
    if data == "instructor_1":
        ctx.user_data[user_id]["instructor"] = "evgeniy"
        # Для Евгения Курочкина отправляем видео, чтобы оно воспроизводилось
...
async def handle_challenges(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user_challenges.get(user_id):
        ...
        await query.message.reply_text(f"Для доступа к челленджам нужно 300 баллов.\nУ вас: {user_scores.get(user_id, 0)} баллов.\nПродолжайте тренировки!")
...
# ОБРАБОТЧИКИ ПЛАТНОГО КУРСА (ЛОГИРОВАНИЕ ДОБАВЛЕНО)
async def handle_paid_course(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    logger.info("handle_paid_course called for user %s", user_id)
    discount = min(user_scores.get(user_id, 0) * 2, 600)
    price = 2000 - discount
    await query.message.reply_text(f"📚 **Платный курс** 📚\n\nСтоимость курса: 2000 рублей.\nВаша скидка: {discount} рублей.\nИтоговая сумма: {price} рублей.\n\nПереведите сумму на карту: 89236950304 (Яндекс Банк).\nПосле оплаты отправьте чек для проверки.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Отправить чек", callback_data="send_receipt")]]))
...
async def handle_send_paid_receipt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    day = int(query.data.split("_")[-1])
    logger.info("handle_send_paid_receipt called for user %s, day %s", user_id, day)
    ctx.user_data.setdefault(user_id, {})["waiting_for_paid_receipt"] = True
...
async def handle_paid_next_day(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    logger.info("handle_paid_next_day called for user %s", user_id)
    day = ctx.user_data[user_id].get("paid_current_day", 1)
    if day < 5:
        ctx.user_data[user_id]["paid_current_day"] = day + 1
        await start_paid_course(query.message, ctx, user_id)
    else:
...
async def start_paid_course(msg, ctx: ContextTypes.DEFAULT_TYPE, user_id: int):
    day = ctx.user_data[user_id].get("paid_current_day", 1)
    logger.info("start_paid_course called for user %s, day %s", user_id, day)
    if day > 5:
    ...
    paid_program = {
        ...
        2: ["Косые скручивания 3х30+10 https://t.me/c/2241417709/284/285",
        ...
        3: ["Подъёмы ног 3х15+5 https://t.me/c/2241417709/270/271",
        ...
        4: ["Скручивания 3х20+10 https://t.me/c/2241417709/379/380",
...
async def handle_paid_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    user_name = update.message.from_user.first_name
    day = ctx.user_data[user_id].get("paid_current_day")
    if day:
        logger.info("handle_paid_video: user %s, day %s", user_id, day)
...
# Остальной функционал (КБЖУ, Мой кабинет, Обо мне, заработок/расход баллов)
...
async def handle_my_cabinet(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    score = user_scores.get(user_id, 0)
    stat = user_status.get(user_id, statuses[0])
...
# Обработчики платного курса для Евгения Курочкина с логированием
async def start_paid_course(msg, ctx: ContextTypes.DEFAULT_TYPE, user_id: int):
    day = ctx.user_data[user_id].get("paid_current_day", 1)
    logger.info("start_paid_course: user_id=%s, day=%s", user_id, day)
    if day > 5:
        return await msg.reply_text("Поздравляем! Вы завершили платный курс! 🎉", reply_markup=main_menu())
    # Программа платного курса уже определена выше (paid_program)
    # Для упрощения используем ту же переменную paid_program внутри функции
    paid_program_local = {
        1: ["Махи назад с утяжелителями 3х25+5 https://t.me/c/2241417709/337/338",
            "Выпады 3х30 шагов х 2кг https://t.me/c/2241417709/157/158",
            "Разведение ног 3х20 https://t.me/c/2241417709/128/129",
...
async def handle_send_paid_receipt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    day = int(query.data.split("_")[-1])
    logger.info("handle_send_paid_receipt: user_id=%s, day=%s", user_id, day)
    ctx.user_data.setdefault(user_id, {})["waiting_for_paid_receipt"] = True
    await query.message.reply_text("Пожалуйста, отправьте фото чека об оплате.")
...
async def handle_paid_next_day(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    logger.info("handle_paid_next_day: user_id=%s", user_id)
    day = ctx.user_data[user_id].get("paid_current_day", 1)
    if day < 5:
        ctx.user_data[user_id]["paid_current_day"] = day + 1
        await start_paid_course(query.message, ctx, user_id)
...
async def handle_paid_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    day = ctx.user_data[user_id].get("paid_current_day")
    if day:
        logger.info("handle_paid_video: user_id=%s, day=%s", user_id, day)
        await ctx.bot.send_message(chat_id=GROUP_ID, text=f"Платный видео-отчет от {user_name} (ID: {user_id}) за день {day}.")
        await ctx.bot.send_video(chat_id=GROUP_ID, video=update.message.video.file_id)
