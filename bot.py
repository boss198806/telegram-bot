...
async def handle_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_waiting_for_video:
        ...
        await ctx.bot.send_message(chat_id=GROUP_ID, text=f"Видео-отчет от {user_name} (ID: {user_id}) за челлендж.")
        await ctx.bot.send_video(chat_id=GROUP_ID, video=update.message.video.file_id)
        user_scores[user_id] += 60
...
async def send_challenge_task(message, ctx_user_id: int):
    day = user_challenges[ctx_user_id]["current_day"]
    exercises = {
...
async def handle_receipt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await ctx.bot.send_photo(chat_id=GROUP_ID, photo=photo_id, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Подтвердить", callback_data=f"confirm_payment_{user_id}")]]))
