...
user_waiting_for_receipt = {}  # Ожидание фото чека (для платного курса)
user_challenges = {}       # Для челленджей
statuses = ["Новичок", "Бывалый", "Чемпион", "Профи"]

# Главное меню – кнопки расположены в два столбца для компактности
def main_menu():
    buttons = [
...
async def handle_paid_course(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    price = 2000 - discount
    instructor = ctx.user_data[user_id].get("instructor")
    # Если тренер Евгений – используем стандартный callback, иначе (например, Анастасия) другой
    if instructor == "evgeniy":
        callback_receipt = "send_receipt"
    else:
        callback_receipt = "send_receipt_anastasiya"
...
# Обработка нажатия кнопки "Отправить чек" для Евгения
...
# Обработка нажатия кнопки "Отправить чек" для Анастасии
...
async def handle_receipt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await ctx.bot.send_photo(
        chat_id=GROUP_ID, photo=photo_id,
