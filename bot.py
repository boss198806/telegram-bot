...
async def handle_send_report(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Если отчет за этот день уже обработан – просто ничего не делаем
...
async def handle_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Если флаг ожидания видео отсутствует – считаем, что отчет уже обработан
    ...
    # Обработка платного курса (если data – кортеж)
    ...
    # Обработка бесплатного курса (data – число)
    elif isinstance(data, int):
        day = data
        # Если отчет уже зафиксирован – не обрабатываем повторно
        if user_reports_sent.get(user_id, {}).get(day):
            return
