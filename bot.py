...
# Глобальные словари (баллы, статусы, отчёты, ожидания)
user_scores = {}           # Для пользователей (инструкторские баллы хранятся отдельно)
user_status = {}
free_reports = {}          # Отчёты по бесплатному курсу
paid_reports = {}          # Отчёты по платному курсу
free_waiting_for_video = {}    # Ожидание видео по бесплатному курсу: {user_id: day}
paid_waiting_for_video = {}    # Ожидание видео по платному курсу: {user_id: day}
...
# Главное меню – кнопки в два столбца для компактности
def main_menu():
    buttons = [
...
async def start_free_course(msg, ctx: ContextTypes.DEFAULT_TYPE, user_id: int):
    # Для тренеров Евгений и Анастасия используем индивидуальные ключи для бесплатного курса
    ...
    if instructor in ["evgeniy", "anastasiya"]:
        ...
        key = f"{instructor}_free_day"
        if key not in ctx.user_data[user_id]:
            ctx.user_data[user_id][key] = 1
        day = ctx.user_data[user_id][key]
    else:
        if not (ctx.user_data[user_id].get("gender")=="female" and ctx.user_data[user_id].get("program")=="home"):
            return await msg.reply_text("Пока в разработке 🚧", reply_markup=main_menu())
    ...
    photos = {1:"https://github.com/boss198806/telegram-bot/blob/main/IMG_9647.PNG?raw=true",
...
async def confirm_payment(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_waiting_for_receipt: del user_waiting_for_receipt[user_id]
    ...
    if ctx.user_data[user_id].get("instructor") == "evgeniy":
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("👨 Мужчина", callback_data="paid_gender_male"),
...
async def handle_paid_next_day(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if paid_day < 5:
        ...
        paid_program = {
            ...
            5: ["Вместо дня 5 оставим пример или финальную программу 🏆"]
        }
...
async def handle_spend_points(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    score = ctx.user_data[user_id].get(f"{ctx.user_data[user_id].get('instructor')}_score", 0) if ctx.user_data[user_id].get("instructor") in ["evgeniy", "anastasiya"] else ctx.user_data[user_id].get("global_score", 0)
