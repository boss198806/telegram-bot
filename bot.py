...
# Для новой функции «Подсчет КБЖУ» будем хранить данные в user_data[user_id]["kbju"]
# и флаги ожидания ввода текстовых данных.
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🍎 Подсчет КБЖУ", callback_data="calc_kbju")],
    ])
...
# --------------------- Новая функция: Подсчет КБЖУ ---------------------
# После нажатия кнопки "🍎 Подсчет КБЖУ" в главном меню
async def handle_calc_kbju(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text("Эта функция «Подсчет КБЖУ» стоит 300 баллов. Хотите купить?", reply_markup=kb)
...
async def handle_buy_kbju(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Инициализируем данные для КБЖУ
    ctx.user_data[user_id]["kbju"] = {}
    # Просим выбрать пол для расчёта КБЖУ
    kb = InlineKeyboardMarkup([
...
async def handle_kbju_age_height_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if ctx.user_data.get(user_id, {}).get("awaiting_kbju_age_height"):
        try:
            ...
            # Просим выбрать уровень активности
            kb = InlineKeyboardMarkup([
            ...
            await update.message.reply_text("Выберите уровень вашей активности:", reply_markup=kb)
        except Exception as e:
        ...
        # Если не ожидаем этих данных, пропускаем
        pass
...
async def handle_kbju_weight_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if ctx.user_data.get(user_id, {}).get("awaiting_kbju_weight"):
        try:
...
async def handle_kbju_goal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "kbju_goal_maintain":
        ...
        goal = "lose"
    elif query.data == "kbju_goal_gain":
        goal = "gain"
    ctx.user_data[user_id]["kbju"]["goal"] = goal
    # Выполняем расчёт (здесь приводим тестовый результат)
    kbju_data = ctx.user_data[user_id]["kbju"]
    # Здесь можно вставить реальную формулу расчёта КБЖУ на основе данных
    ...
    # Очистим данные по КБЖУ
    ctx.user_data[user_id].pop("kbju", None)

# --------------------- /start и выбор инструктора (остальное без изменений) ---------------------
async def handle_instructor_selection(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    data = query.data
    user_id = query.from_user.id
...
# --------------------- Остальной функционал (Меню питания, рефералка, челленджи и т.п.) ---------------------
...
async def handle_kbju_goal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data[user_id]["kbju"]["goal"] = goal
    kbju_data = ctx.user_data[user_id]["kbju"]
    # Тестовый расчёт (здесь можно вставить реальную формулу)
    result = (f"Ваш расчет КБЖУ:\n"
              f"Калорий: 2200 ккал\n"
              f"Белков: 150 г\n"
              f"Жиров: 70 г\n"
...
# --------------------- Новая функция: Подсчет КБЖУ (в отдельной кнопке) ---------------------
async def handle_calc_kbju(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    kb = InlineKeyboardMarkup([
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_kbju_age_height_text))
    app.add_handler(CallbackQueryHandler(handle_kbju_activity, pattern="^kbju_activity_"))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_kbju_weight_text))
