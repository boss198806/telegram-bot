...
async def start_free_course(message, context: ContextTypes.DEFAULT_TYPE, user_id: int, instructor: str):
    ...
    # Условие доступа: только для женщин с программой "дома"
    if gender != "female" or program != "home":
        await message.reply_text("Бесплатный курс доступен только для женщин с программой 'дома'.", reply_markup=main_menu())
        return
    ...
    # Программа для тренера Анастасии
    if instructor == "anastasiya":
        ...
        await message.reply_text("Программа для этого тренера пока в разработке.", reply_markup=main_menu())
        return
