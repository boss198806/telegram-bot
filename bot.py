...
# Основное меню (добавлена кнопка "Подсчет КБЖУ")
def main_menu():
...
# 1. Функции бесплатного курса (без изменений)
...
async def handle_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in user_waiting_for_video:
        ...
        if day < 5:
            ...
            new_day = ctx.user_data[uid]["current_day"]
            user_waiting_for_video[uid] = new_day
            await update.message.reply_text(f"Отчет за день {day} принят! 🎉\nВаши баллы: {user_scores[uid]}.\nГотовы к следующему дню ({new_day})?",
    ...
    elif uid in user_waiting_for_challenge_video:
        await ctx.bot.send_message(chat_id=GROUP_ID, text=f"Видео-отчет от {uname} (ID: {uid}) за челлендж.")
        await ctx.bot.send_video(chat_id=GROUP_ID, video=update.message.video.file_id)
        user_scores[uid] += 60
        del user_waiting_for_challenge_video[uid]
...
# 2. Обработчики пола/программы (бесплатного курса) – без изменений
...
# 3. Функция /start и выбор инструктора
...
async def handle_instructor_selection(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "instructor_1":
        ...
        # Отправляем видео сразу, чтобы оно воспроизводилось
...
# При нажатии кнопки "Подсчет КБЖУ"
...
async def handle_kbju_activity(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    uid = q.from_user.id
    activity = q.data.split("_")[-1]  # low, medium, high
    ctx.user_data.setdefault("kbju", {})["activity"] = activity
...
async def handle_kbju_goal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    result = (f"Ваш расчет КБЖУ:\nКалорий: {calories:.0f} ккал\nБелков: {protein:.0f} г\nЖиров: {fat:.0f} г\nУглеводов: {carbs:.0f} г")
    await q.message.reply_text(result, reply_markup=main_menu())
...
async def send_challenge_task(msg, uid: int):
    ...
    ex = {
        ...
        2: ["1️⃣ Присед со штангой (можно без) 30 раз [Видео](https://t.me/c/2241417709/140/141)",
        ...
        5: ["1️⃣ Присед со штангой (можно без) 50 раз [Видео](https://t.me/c/2241417709/140/141)",
